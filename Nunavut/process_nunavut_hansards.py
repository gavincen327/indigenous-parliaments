    ...
    if oral_questions:
        ...
        for idx in range(len(question_list)):
            if title in question_list[idx]:
                print('Title in idx:', idx)
                # print(idx, 'entry:', question_list[idx])
                if idx > 0:
                    print('Previous entry:', question_list[idx-1])
                if idx < len(question_list)-1:
                    print('Next entry:', question_list[idx+1])
                if idx == 0:
        ...
        #     question_list[0] = first_item.replace(title, '')
...
def main():
    ...
    # path_dct = csv_2_date_path_dict('Nunavut/archives_1st_special.csv')
    for dte in path_dct:
